...
unsigned long long dbg_mc_now;
unsigned long long dbg_mc_duration;
unsigned long long dbg_mc_after;
unsigned long long dbg_mcr_1, dbg_mcr_2;
unsigned long long dbg_mc_counter;

void dbg_dump_sleep(void)
{
	puth(dbg_mc_now, 16);
	putc(' ');
	puth(dbg_mc_duration, 16);
	putc(' ');
	puth(dbg_mc_after, 16);
	putc(' ');
	puth(dbg_mcr_1, 16);
	putc(' ');
	puth(dbg_mcr_2, 16);
	putc(' ');
	puth(dbg_mc_counter, 8);
	puts("\r\n");
}
...
void sleep(unsigned long long us)
{
	...
	dbg_mc_duration = mc_duration;
	unsigned long long mc_after = mc_now + mc_duration;
	dbg_mc_after = mc_after;
	...
	while (1) {
		...
		dbg_mc_counter++;
		if (dbg_mcr_2 >= mc_after) {
			/* volatile unsigned long long _wait = 100; */
			/* while (_wait--); */
			/* dbg_dump_sleep(); */
			putc('C');
			break;
